...
2. Go to your repository at [mikejmets/qa-webtide-on-vm21](https://github.com/mikejmets/qa-webtide-on-vm21).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `qa.webtide.co.za` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | qa.webtide.co.za | The hostname or IP address of your Docker Swarm manager. |
...
- [Backend Tests Workflow](https://github.com/mikejmets/qa-webtide-on-vm21/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/mikejmets/qa-webtide-on-vm21/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`ghcr.io/mikejmets/qa-webtide-on-vm21-backend`) and Frontend (`ghcr.io/mikejmets/qa-webtide-on-vm21-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of qa.webtide.co.za](https://github.com/mikejmets/qa-webtide-on-vm21/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/qa.webtide.co.za.yml` stack.

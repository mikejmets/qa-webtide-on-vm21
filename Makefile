...
PROJECT_NAME=qa-webtide-on-vm21
STACK_NAME=qa-webtide-co-za
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://qa-webtide-on-vm21.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t mikejmets/qa-webtide-on-vm21-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t mikejmets/qa-webtide-on-vm21-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name qa-webtide-on-vm21-frontend-acceptance --link qa-webtide-on-vm21-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d mikejmets/qa-webtide-on-vm21-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name qa-webtide-on-vm21-backend-acceptance -d mikejmets/qa-webtide-on-vm21-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop qa-webtide-on-vm21-frontend-acceptance
	@docker stop qa-webtide-on-vm21-backend-acceptance
